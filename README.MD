...
|instance_type | map of k8s instance types | map(string) | etcd=t2.large,control-plane=t2.large,worker=t2.large |
| root_block_device | root block device configuration | list(map(string)) | | no |
